...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (authHeader) {
      ...
      if (!authError && user) {
        const rateLimitResponse = await checkRateLimit(supabase, user.id, 'send-deletion-request-notification');
    ...
    const notificationPromises = adminIds.map(adminId => 
      supabase.rpc('create_notification', {
        p_user_id: adminId,
        p_title: 'New Deletion Request',
        p_message: `${requestedByEmail} has requested to delete a ${tableName} record.${reasonText}`,
        p_link_url: '/settings',
